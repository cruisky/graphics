...
namespace TX {
	namespace SSE {
		...
		public:
			...
			inline const V4Float operator - () const { return _mm_xor_ps(m, _mm_castsi128_ps(SIGN_MASK[0xF])); }
			...
			inline const bool operator == (const V4Float& ot) const { return _mm_movemask_ps(_mm_cmpeq_ps(m, ot.m)) == 0xF; }
			inline const bool operator == (const float& ot) const { return *this == V4Float(ot); }
			inline const bool operator != (const V4Float& ot) const { return _mm_movemask_ps(_mm_cmpeq_ps(m, ot.m)) != 0xF; }
			inline const bool operator != (const float& ot) const { return *this != V4Float(ot); }
		...
		inline const V4Float Abs(const V4Float& v){ return _mm_andnot_ps(_mm_castsi128_ps(SIGN_MASK[0xF]), v.m); }
		...
		inline const V4Float Blend(const V4Float& a, const V4Float& b, const V4Bool& fb){ return _mm_blendv_ps(a.m, b.m, _mm_castsi128_ps(fb)); }
